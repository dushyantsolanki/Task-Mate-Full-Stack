...
export default function OtpVerifyForm({
    ...
    const handleOTPVerification = async (values: string) => {
        setIsLoading(true)
        try {
            const response = await axios.post(`${import.meta.env.VITE_API_BASE_URL}/auth/verify-otp`, { otp: values, email })
    ...
    const handleResendOtp = async () => {
        if (canResend) {
            setIsLoading(true)
            try {
                const response = await axios.post(`${import.meta.env.VITE_API_BASE_URL}/auth/resend-otp`, { email, purpose: 'verify_account' })
                const data = await response.data
                if (response.status === 201) {
                    toast.success(data.message)
